...
class IBMSentimentScorer:
    ...
    def __init__(
        ...
        # Set up directories
        self.csv_list = os.listdir('./output/{}/reviews'.format(
            self.target_folder
        ))
        ...
        if self.API_options=="aspects":
            if self.continue_in_folder is None:
                self.sentiment_folder_path = './output/{}/sentiments_{}/'.format(
                    self.target_folder,
            ...
            if self.continue_in_folder is None:
                self.overall_folder_path = './output/{}/overall_{}/'.format(
                    self.target_folder,
                    datetime.now().strftime('%y%m%d_%H%M%S')
                )
    ...
    def score_sentiments_1_poi(self):
        ...
        while len(self.current_reviews_df.index) > 0:
            ...
            try:
                ...
                log = open('./output/{}/log(API).txt'.format(self.target_folder), 'a+')
