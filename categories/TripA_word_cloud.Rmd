...
get_dtm <- function(docs) {
  ...
  stopwords <- stopwords("english" ,source = "stopwords-iso")
...
tripa_aspects_df <- data.frame(matrix(nrow = 0, ncol = 4))
names(tripa_aspects_df) <- c("POI_INDEX", "ASPECTS", "ATTRACTION_TYPE", "ASPECTS_CRAWLED_TIME")
...
tripa_aspects_df <- subset(tripa_aspects_df, ASPECTS != "")

summary(tripa_aspects_df)
```

```{r}
## Parse POI attraction types from TripA
...
poi_attraction_types <- tripa_aspects_df[match(unique(tripa_aspects_df$POI_INDEX), tripa_aspects_df$POI_INDEX),]
poi_attraction_types <- poi_attraction_types[, c(1, 3)]
poi_attraction_types$ATTRACTION_TYPES <- strsplit(poi_attraction_types$ATTRACTION_TYPES, ", ")

poi_attraction_types_temp <- data.frame(matrix(nrow = 0, ncol = 2))
names(poi_attraction_types_temp) <- c("POI_INDEX", "ATTRACTION_TYPE")

for (i in 1:dim(poi_attraction_types)[1]) {
  df <- data.frame(
    POI_INDEX = poi_attraction_types[i,][1],
    ATTRACTION_TYPE = poi_attraction_types[i,][[2]])
  names(df) <- c("POI_INDEX", "ATTRACTION_TYPE")
  poi_attraction_types_temp <- rbind(poi_attraction_types_temp, df)
}

poi_attraction_types <- poi_attraction_types_temp
row.names(poi_attraction_types) <- NULL
head(poi_attraction_types)
```
