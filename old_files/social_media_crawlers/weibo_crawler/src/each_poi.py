...
def get_proxies():
    ...
    #API_URL = "http://api.scraperapi.com?api_key=e38e94bd8b28849f58569dbe77af4e72&url=http://httpbin.org/ip"
    API_URL = "http://list.didsoft.com/get?email=jironghuang88@gmail.com&pass=yawh82&pid=socks200&version=socks4"

    #payload = {'api_key': 'e38e94bd8b28849f58569dbe77af4e72', 'url':
#'https://httpbin.org/ip'}
    ...
    for proxy_text in proxies_text:
        ...
        if len(proxy_split[0]) > 2:
            #proxy = {"https": (proxy_split[1] + "://" + proxy_split[0])}   #Works on Prof Tung Proxy
            proxy = {"https": ("socks4://" + proxy_split[0])}           #change url string according to proxy list
...
def proxy_check_simple(proxies):
    ...
    for proxy in proxies:
        ...
        if len(good_proxies) == 2:   #If find 20 proxies, stop and return 
            break
        # if count == 11:
        #     break
    return good_proxies
...
file = pd.read_csv('./data/poi_list.csv')
proxies = get_proxies()  #Required in proxies
good_proxies = proxy_check_simple(proxies) #required in proxy
...
def write_first(dic):
    df = pd.DataFrame(dic)
    df.to_csv('./output/weibo_data.csv', encoding='utf_8_sig', quoting=csv.QUOTE_ALL, index=False)
...
def write_file(dic):
    old_df = pd.read_csv('./output/weibo_data.csv')
