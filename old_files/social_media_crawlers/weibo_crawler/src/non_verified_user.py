...
# In cards 0
account_data = {'昵称': 'screen_name', '简介': 'description', '等级': 'rank', '注册时间':'registered time','阳光信用':'credit','verified_type':'verified_type'
                }
...
'junior high school', '小学': 'primary_school', '公司': 'company', '家乡': 'hometown', '海外': 'oversea???',
                 '感情状况': 'relationship', '昵称': 'nickname', '简介': 'description', '中专技校': 'technical school','审核时间': 'company_verified_time',
                 '高职': 'vocational school'}

influence_data = {'weibo_yesterday_number':'weibo_yesterday_number', 'weibo_yesterday_interaction':'weibo_yesterday_interaction', 'review_yesterday_interaction':'review_yesterday_interaction',
            'story_yesterday_interaction':'story_yesterday_interaction'}

places_been_to_data = {'total':'total_country_number', 'title_sub': 'country', 'last_check_in_time': 'last_check_in_time',
...
# This means the user doesn't have this data or the crawler is banned from the place_data_api, not sure
def handle_empty_places_data():
    if len('total_country)number') != 0:
        dic['total_country_number'].append('none_e')
        dic['country'].append('none_e')
        dic['last_check_in_time'].append('none_e')
        dic['check_in_times'].append('none_e')
...
if __name__ == '__main__':
    ...
    file = pd.read_csv('./data/target_user.csv')
    ...
    for index, u in enumerate(file['user_id']):
        ...
        try:
            ...
            extract_influence_data(inf_api)
