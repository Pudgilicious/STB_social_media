...
class QYCrawler:
    ...
    def __init__(self, chromedriver_path, poi_df, cnx, db_out_flag):
        ...
        self.current_page = None                                               #record the current page 
        self.current_poi_index = None                                          #for reviews
        self.current_poi_name = None
        self.current_poi_url = None
        self.review_final_page = False                                         #reach the last page of review
        ...
        self.sel = None                                                        #selector path for each POI
        self.last_page_number=None                                             #number of review pages for each POI

        #to deal with lagging problem
        self.count=1                                                           #to count the number of reloading
                                                                
        # Create unique CSVs.
        self.datetime_string = datetime.now().strftime('%y%m%d_%H%M%S')
        os.makedirs('./qiongyou/output/{}/'.format(self.datetime_string))
        os.makedirs('./qiongyou/output/{}/reviews'.format(self.datetime_string))
        self.attributes_df.to_csv('./qiongyou/output/{}/attributes.csv'
    ...
    def crawl_pois(self, number_of_pages=None):
        ...
        while len(self.poi_df.index) > 0:
            ...
            if self.fsm_state == 1:
                self.current_poi_index = self.poi_df.iloc[0][0]                #read the 1st row of csv input
            ...
            if self.fsm_state == 3:                                            #put logic error in readme. file
                self.fsm_state=2
            sleep(1+random()*2)
            ...
            if self.fsm_state != 3:
                ...
                try:
                    ...
                    if  res == []:                                             #capture broken link and skip t=it
                        self.count+=1
                        log_file = open('./qiongyou/output/{}/log.txt'
                    ...
                    if not self.reviews_crawled:
                        ...
                        self.current_page = None                               #reset the values for next POI
                        self.attributes_crawled = False
                        self.reviews_crawled = False
                    
                    self.poi_df = self.poi_df.iloc[1:]                         #remove the poi(first row) from df once crawled
        
                        
                   
                except:
                    self.fsm_state=3                                           #crashed happend,raise Exception
                    self.driver.quit()                                         #note down the error, recrawl
                    ...
                    if self.reviews_crawled==True:
                        os.remove('./qiongyou/output/{}/reviews/{}.csv'
                                           .format(self.datetime_string,
                                                   self.current_poi_index)) 
    ...
    def crawl_reviews(self):
        ...
        page_list=res.xpath(xpath_last_page).getall()
        if len(page_list)>1:                                                   #get the last page_number for the poi,which is the second last element
            self.last_page_number=int(page_list[-2])                           #example(...88)
        else:
    ...
    def crawl_reviews_1_page(self, poi_index): 
        ...
        if next_page_button:
            if self.driver.current_url!= self.current_poi_url:                 #wrong page but with next_page button
    ...
    def crawl_agg_list(self):
        ...
        poi_df = pd.DataFrame(columns=['WEBSITE_INDEX',
                                       'POI_NAME',
                                       'ENGLISH_NAME',
                                       'URL'])
        
        # There are 24 pages of attractions in QY.                             #config file/x path
        for i in range(1,25):
                ...
                for a in range (1,16):
                    ...
                    website_index=4
                    link =str(res.xpath(xpath_link).extract_first())     #eg. link = res.xpath('div[1]/div/div/div/div[2]/a[@target="_blank"]/@href').extract_first()       
                    text =str( res.xpath(xpath_text).extract_first())  
                    eng =str(res.xpath(xpath_eng).extract_first() ) 
                    reviews = str(res.xpath(xpath_reviews).extract_first() )
                    
                  
                    poi_df.loc[poi_df.shape[0]] = [website_index] + [text] + [eng] + [link] #+ [review_number]
        ...
        self.driver.quit()
        poi_df['POI_NAME'] = poi_df['POI_NAME'].apply(lambda x: x.replace(' ',''))    
        poi_df['URL'] = poi_df['URL'].apply(lambda x: 'https://'+x[2:])
        
        #Write out as data-frame with date
        today = date.today()
        today_date = today.strftime("%Y%m%d")
        poi_df.to_csv("/home/jia/Desktop/git/STB_social_media_analytics/experimentation/jiaxin_experiment/QY/{}_qy_list.csv".format(today_date))
